...
class msSalePrice
{
	...
	public function getPrice($product = 0, $count = 1, $data = array())
	{
		...
		if( $mssp = $this->modx->getObject('msspPrice', $q) ){
			$mssp_price = $this->getPriceByType($mssp->get('type'), $mssp->get('price'), $default_price);
			$output = array(
				'rid'		=>	$product->get('id'),
				'price'		=>	$mssp_price,
        ...
        $response = $this->ms2->invokeEvent('msspOnAfterGetPrice', array(
            ...
            'rid'		=> $product->get('id'),
            'count'		=> $count,
            'price'		=> $mssp->ms2->formatPrice($output['price']),
            'old_price'	=> $mssp->ms2->formatPrice($output['old_price']),
			'data' 		=> $data,
        ));
        if (!$response['success']) {
            return $this->error($response['message']);
