...
<div class="main my-4">
        ...
        <h1 class="text-uppercase font-weight-bold mb-4">{$_modx->resource.pagetitle}</h1>
            ...
            {set $section_category_bottom_id = $_modx->resource.section_category_bottom | fromJSON}
            {set $section_category_bottom}
            {foreach $section_category_bottom_id as $v last=$last}
                {if $last ?}
                {$v | resource : 'id'}
                {else}
                {$v | resource : 'id'},
